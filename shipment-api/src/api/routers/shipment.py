...
async def assign_shipment_to_courier(
    ...
    if await user_service.get_user_by_id(courier_id):
        if shipment := await shipment_service.assign_shipment_to_courier(
            shipment_id, courier_id
        ):
            return shipment
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND,
        detail="Shipment not found or wrong courier id",
    )
...
async def check_status(
    ...
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND,
        detail="Shipment not found or wrong recipient email",
    )


@router.get(
    "/all", response_model=Iterable[ShipmentDTO], status_code=status.HTTP_200_OK
)
@inject
...
@router.get(
    "/get/{shipment_id}", response_model=ShipmentDTO, status_code=status.HTTP_200_OK
)
@auth.role_required(UserRole.COURIER)
@inject
async def get_shipment(
    ...
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND,
        detail="No shipment found with the provided ID.",
    )
...
async def delete_shipment(
    ...
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND,
        detail="No shipment found with the provided ID. Try again.",
    )
...
async def sort_by_distance(
    ...
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND,
        detail="No shipments found with this courier.",
    )
...
@router.put(
    "/update/{shipment_id}", response_model=ShipmentDTO, status_code=status.HTTP_200_OK
)
...
async def update_shipment(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Shipment not found or invalid data",
        )
    except ValueError as error:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=str(error)
